...
//  ForgottenRealmsMap
//
//  Created by Joey Marino on 5/10/18.
//  Copyright © 2018 L&L Programming. All rights reserved.
//

import UIKit

class DetailsOfFaerun: UIView {
    ...
    func addDetailView() {
        ...
        titleLabel.text = FaerunViewController().name()
        titleLabel.font = UIFont.boldSystemFont(ofSize: 16.0)
        scrollView.addSubview(titleLabel)
        
        let imageName = FaerunViewController().imageName()
        ...
        visitWikiButton.addTarget(FaerunViewController(), action: #selector(FaerunViewController.visitWiki), for: .touchUpInside)
        ...
        let zoomable = FaerunViewController().zoomable()
        if zoomable == true {
        ...
        zoomButton.addTarget(FaerunViewController(), action: #selector(FaerunViewController.zoom), for: .touchUpInside)
        ...
        descriptionLabel.text = FaerunViewController().message()
